...
> **⚠️ Corrections (24 September 2026).** The table below is STALE — it disagrees with
> the artifact it claims to summarise. `pnpm run eval:gap` currently writes:
>
> | Size | Greedy assigned | Optimal assigned | Greedy score | Optimal score | **Ratio** |
> |---|---|---|---|---|---|
> | 10×3 | 2 | 2 | 0.934500 | 0.934500 | **1.0000** |
> | 25×8 | 10 | 10 | 4.994716 | 4.998755 | **0.9992** |
> | 50×16 | 25 | 25 | 12.975307 | 12.977508 | **0.9998** |
> | 100×33 | **59** | **64** | 33.408870 | 35.411389 | **0.9435** |
>
> So the true 100×33 result is 59 of 64 placements at 94.35% of the optimal score,
> not 81 of 81 at 99.91% — the stale row was generated by an earlier code state and
> is also the origin of the contradiction with `benchmarks/EVALUATION_FINDINGS.md` §6.
> The paragraph after the table repeats the same error and is superseded.
>
...
> **Correction on the ½ claim (24 September 2026).** The ½-approximation theorem
> does not cover this engine as deployed. Its priority mixes a fairness term into
> the ordering (`greedy-assignment.engine.ts:307`), `topK` truncates candidate
> edges (`:122-123`), and equal scores are broken by an FNV-1a hash — three
> assumptions the classical proof needs. The bound applies only to the static-only
> variant (δ=0, `topK = ∞`, no hash tie-break); the measured ratios are empirical.
> The measured scaling claim above also deserves a caveat: the engine's own
> wall-clock was dominated by pair scoring (~1.5 s at 5000×500), and a
> `performance.now()`-measured p50/p95 has since replaced the single `Date.now()`
> reading quoted here.
