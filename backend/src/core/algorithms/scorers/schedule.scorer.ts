...
export class ScheduleScorer {
  public score(student: Student, tutor: Tutor): number {
    ...
    if (requestedMinutes <= 0) {
      throw new IncompleteProfileException(student.id, 'student', 'requestedAvailability');
    }

    const overlappingMinutes = student.requestedAvailability.reduce((studentTotal, studentSlot) => {
