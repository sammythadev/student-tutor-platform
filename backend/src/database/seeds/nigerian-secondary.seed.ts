...
/**
 * Seeds the demo subjects, tutors, students and availability slots. Idempotent:
 * re-running skips existing rows, so it is safe to call on every boot.
 */
export async function runNigerianSecondarySeed(db: NodePgDatabase): Promise<void> {
...
/** CLI entry point: `pnpm run db:seed`. */
async function seed(): Promise<void> {
  loadEnvironmentFiles();
  const pool = new Pool({ connectionString: getDatabaseUrl() });

  try {
    await runNigerianSecondarySeed(drizzle(pool));
  } finally {
    await pool.end();
  }
}

// Only seed when this file is the entry point. `runNigerianSecondarySeed` is also
// imported by the boot-time setup, and the CLI wrapper must not run there.
