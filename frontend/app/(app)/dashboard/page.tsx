    ...
    <div className="space-y-7 py-3">
      <div className="rounded-2xl p-7 bg-primary text-white shadow-sm">
        ...
        <div className="mt-2 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            <h1 className="font-heading text-3xl font-bold">Welcome back, {user?.firstName ?? 'Student'}</h1>
            <p className="mt-1 text-sm text-white/75">
              {metrics ? `${metrics.streakDays} day streak, ${totalHours.toFixed(1)}h logged this week` : 'Loading your learning snapshot'}
            </p>
          </div>
        ...
        <div className="surface-card p-6">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex flex-wrap gap-2">
              {(studentProfile?.subjects ?? [studentProfile?.requiredSubject].filter(Boolean) as string[]).map(subject => (
