...
import { CheckCircle2, Heart, Search, SlidersHorizontal, Star } from 'lucide-react'
...
export default function TutorsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (alive) setTutors(page.candidates)
      } catch (err: any) {
  ...
  const subjects = useMemo(() => {
    ...
    tutors.forEach(tutor => tutor.subjectsTaught?.forEach(item => unique.add(item)))
    return ['All Subjects', ...Array.from(unique).sort()]
  }, [tutors])

  const filtered = tutors.filter(tutor => {
    const q = search.toLowerCase()
    const matchesSearch = !q || `${tutor.firstName} ${tutor.lastName}`.toLowerCase().includes(q) || tutor.subjectsTaught.some(item => item.toLowerCase().includes(q))
    const matchesSubject = subject === 'All Subjects' || tutor.subjectsTaught.includes(subject)
  ...
  return (
    <div className="space-y-8 py-3">
      <div>
        ...
        <p className="mt-1 text-sm text-text-secondary">Browse {filtered.length} matched tutors from the backend ranking endpoint</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map((tutor, index) => {
            ...
            const isLiked = liked.has(tutor.userId)
            return (
              <Card key={tutor.userId} className="flex flex-col p-5">
                  ...
                  <div className="flex min-w-0 items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex items-center gap-1.5">
                        ...
                        <span className="text-xs font-semibold text-text-primary">{tutor.avgRating ?? 'New'}</span>
                        <span className="text-xs text-text-muted">({tutor.ratingCount})</span>
                      </div>
                    </div>
                  </div>
                  <button
                    onClick={() => setLiked(prev => {
                      ...
                      next.has(tutor.userId) ? next.delete(tutor.userId) : next.add(tutor.userId)
                      return next
                    })}
                ...
                <p className="mb-4 line-clamp-3 text-xs leading-relaxed text-text-secondary">{tutor.bio ?? 'No bio yet.'}</p>

                <div className="mb-4 flex flex-wrap gap-1.5">
                  {tutor.subjectsTaught.slice(0, 3).map(item => <Badge key={item} color={color as any} size="sm">{item}</Badge>)}
                </div>

                <div className="mb-4 flex items-center justify-between border-y py-3" style={{ borderColor: 'var(--border)' }}>
                  <span className="flex items-center gap-1.5 text-xs text-accent-mint-fg"><CheckCircle2 className="h-3.5 w-3.5" /> Ranked match {Math.round((tutor.score ?? 0) * 100)}%</span>
                  <span className="font-bold text-text-primary">₦{Number(tutor.hourlyRate ?? 0).toLocaleString()}<span className="text-xs font-normal text-text-muted">/hr</span></span>
                </div>

                <div className="mt-auto flex gap-2">
                  <Button variant="secondary" size="md" className="flex-1">View Profile</Button>
                  <Button size="md" className="flex-1" onClick={() => handleBook(tutor)}>Book Session</Button>
                </div>
              </Card>
            )
          })}
