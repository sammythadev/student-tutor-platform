...
const Grainient = ({
  ...
  useEffect(() => {
    ...
    const loop = t => {
      if (lastTime !== null) program.uniforms.iTime.value += (t - lastTime) * 0.001;
      lastTime = t;
      renderer.render({ scene: mesh });
    ...
    const tryStart = () => {
      if (isVisible && isPageVisible && !reduced && raf === 0) raf = requestAnimationFrame(loop);
    };
    const tryStop = () => {
    ...
    const onMotionChange = () => {
      ...
      if (reduced) {
        tryStop();
        // Leave one static frame behind so the field never goes blank.
        renderer.render({ scene: mesh });
      } else {
        tryStart();
      }
  ...
  useEffect(() => {
    ...
    // A paused renderer still needs one paint after a theme or prop change.
