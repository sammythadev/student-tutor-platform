...
The third, deferred acceptance (da-stable), is the student-proposing Gale-Shapley algorithm (Gale & Shapley, 1962) in its college-admissions form with tutor capacities: both sides rank by the same static composite score, and the outcome is a stable matching in which no student–tutor pair would both prefer each other over their final matches. Stability is a different objective from total score, so it is a genuine competing alternative rather than a tuning variant of the engine.
